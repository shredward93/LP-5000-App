...
- **ButterCut** — a git clone (not a gem), auto-detected at `~/Buttercut`, configurable in Settings. Install/pin it with `buttercut_bootstrap.sh` (repo root). `CLAUDE.md` references its resolved path directly for Claude to use its Ruby helpers (contact sheets, transcript extraction, library migrations) as a reference, but multi-angle/B-Roll cuts are hand-authored FCP7 XML per the Track Protocol baked into every generated `CLAUDE.md` — not routed through ButterCut's own (single-track-only) exporter. Use the **Update from GitHub** button in Settings to pull the latest.
- **Single-camera franken-bite XML export** — `assets/xml-export/` is the one source of truth for exporting a social clip made of trimmed ranges from one source (Resolve/Premiere/FCPX). `CLAUDE.md` always points Claude at `franken_bit_export.rb` + `EXPORT_NOTES.md` there directly, so a fix or a newly-confirmed editor target only ever needs updating in this one place, then commit + push.
